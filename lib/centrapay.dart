...
class CentrapayScreenState extends State<CentrapayScreen> {
      ...
      case CentrapayPaymentMethodZapTest:
        ...
        if (tx == null)
          return CentrapayZapResult(true, null,
              CentrapayRequestPayResult(null, CentrapayError.Network));
  ...
  void payConfirm() async {
    if (_req == null || _payment == null || _zapTx == null) return;
    setState(() {
      _loading = true;
      _msg = 'confirming payment..';
    });
    var res =
        await centrapayRequestPay(widget._qr, _req!, _payment!, _zapTx!.id);
    switch (res.error) {
  ...
  void pay() async {
    if (_req == null || _payment == null) return;
    setState(() {
      _loading = true;
      _msg = 'paying request..';
    });
