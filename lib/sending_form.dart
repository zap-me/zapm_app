        ...
        child: Column(
          ...
          children:
            <Widget>[
              ...
              Visibility(
                  visible: !_sending,
                  child: RoundedButton(() => Navigator.pop(context, _tx), ZapBlue, ZapWhite, 'close', borderColor: ZapBlue)
              ),
          ],
        ),
      ),
...
class PayDbSendingFormState extends State<PayDbSendingForm> {
  ...
  void send() async {
    var result = await paydbTransactionCreate(ActionTransfer, widget._recipient, widget._amount, widget._attachment);
    setState(() {
      _sending = false;
      _tx = result.tx;
    });
        ...
        child: Column(
          ...
          children:
            <Widget>[
              ...
              Visibility(
                visible: !_sending && _tx != null,
                child: Text("transaction complete (${_tx?.token})", style: TextStyle(color: ZapBlue)),
              ),
              Visibility(
                visible: !_sending && _tx == null,
